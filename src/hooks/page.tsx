...
export default function TournamentLivePage() {
  ...
  const _handleVoteReceived = React.useCallback((itemId: 'item1' | 'item2') => {
  ...
  const { tournamentId, tournamentTitle, initialItems, isLoadingData, dataError: _dataError, tournamentMode } = useTournamentData();
  ...
  const {
    ...
    activeMatch, startTournament, handleDeclareWinnerAndNext, handleStopTournament, updateScore, modifyScore, isTournamentActive,
  } = useTournamentLogic({ 
    initialItems, 
  ...
  // Hook pour TMI
  const { isTmiConnected, tmiError: _tmiError, votedUsers, superVotesThisMatch: _superVotesThisMatch } = useTmiClient({
    liveTwitchChannel,
    isTournamentActive,
    tournamentWinner,
    activeMatch,
  ...
  useEffect(() => {
    ...
    if (!hasShownRestoreNotification && matches.length > 0) {
  ...
  }, [matches.length, hasShownRestoreNotification, urlChannel, liveTwitchChannel, isLoadingData]);
